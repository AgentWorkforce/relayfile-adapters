...
- `@relayfile/adapter-ramp` now provides read-only Ramp finance materialization with canonical bill, purchase-order, item-receipt, vendor-agreement, transaction, reimbursement, receipt, vendor, transfer, repayment, dimension, and accounting paths; stable indexes and aliases; Hookdeck-aware inbound declarations; webhook normalization and signature verification; and digest-visible lifecycle classification.
