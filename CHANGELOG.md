...
- `@relayfile/adapter-core/inbound` now publishes a version-hashed, provider-owned Nango/Hookdeck capability catalog, deterministic cross-runtime `logicalEventKey`, and exact golden vectors; GitLab Hookdeck prefers `x-gitlab-event-uuid` and falls back to the Hookdeck delivery id.
