...
for (const provider of providerPackages()) {
  ...
  assertActiveTestCoverage(failures, provider, testSource, /deterministic|sorted/i, 'deterministic sorting');
  assertActiveTestCoverage(failures, provider, testSource, /empty/i, 'empty-window behavior');
  assertActiveTestCoverage(
    failures,
    provider,
    testSource,
  ...
  if (appendOnlyLifecycleProviders.has(provider)) {
    assertSourceMentions(failures, provider, digestSource, /append-only|immutable/i, 'documented append-only lifecycle exception');
    assertActiveTestCoverage(failures, provider, testSource, /upsert|upserted/i, 'append-only upsert classification');
  } else {
    assertActiveTestCoverage(
      failures,
      provider,
...
function assertSourceMentions(failures, provider, source, pattern, label) {
  if (!pattern.test(source)) {
    failures.push(`${provider}: digest test must cover ${label}`);
  }
}

export function assertActiveTestCoverage(failures, provider, source, pattern, label) {
  if (!activeTestHasAssertion(source, pattern)) {
    failures.push(`${provider}: digest test must cover ${label} in a live assertion test`);
  }
}
