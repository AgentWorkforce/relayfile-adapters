...
export const digest: DigestHandler = createDigestHandler({
  provider: "asana",
  identify: (canonicalPath) => asanaIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'asana' || digestEventPath(event) === '/asana' || digestEventPath(event).startsWith('asana/') || digestEventPath(event).startsWith('/asana/'))
  );
}
