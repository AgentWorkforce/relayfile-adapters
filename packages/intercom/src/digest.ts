...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${intercomIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'intercom' || digestEventPath(event) === '/intercom' || digestEventPath(event).startsWith('intercom/') || digestEventPath(event).startsWith('/intercom/'))
  );
}
...
const ACTION_VERB_PATTERN_4 = actionVerbRegex('reopen|reopened');
...
const ACTION_VERB_PATTERN_9 = actionVerbRegex('merge|merged');
