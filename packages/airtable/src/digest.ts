...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'airtable' || digestEventPath(event) === '/airtable' || digestEventPath(event).startsWith('airtable/') || digestEventPath(event).startsWith('/airtable/'))
  );
}
...
function airtableIdentifier(path: string): string {
  ...
  if (path.includes('/records/')) return `record ${id}`;
  if (path.includes('/tables/')) return `table ${id}`;
  if (path.includes('/bases/')) return `base ${id}`;
  return id;
}
...
const ACTION_VERB_PATTERN_4 = actionVerbRegex('unarchive|unarchived|restore|restored');
const ACTION_VERB_PATTERN_5 = actionVerbRegex('complete|completed|done');
const ACTION_VERB_PATTERN_6 = actionVerbRegex('cancel|canceled|cancelled');
const ACTION_VERB_PATTERN_7 = actionVerbRegex('resolve|resolved');
const ACTION_VERB_PATTERN_8 = actionVerbRegex('merge|merged');
