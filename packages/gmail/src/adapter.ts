import { GMAIL_NANGO_FALLBACK_SYNC, validateConfig } from './config.js';
import { GmailBridge } from './bridge.js';
...
import type { GmailConfig, FetchContentClient, NangoSyncRecord, ProviderNotification, StorageBridgeEvent, StorageBridgeEventPublisher, WritebackOperation } from './types.js';

export class GmailAdapter {
  readonly slug = "gmail";
  readonly source = "gmail";
  readonly resources = resources;
  readonly nangoFallbackSyncName = GMAIL_NANGO_FALLBACK_SYNC;
  readonly config: GmailConfig;
  private readonly bridge?: GmailBridge;
  private readonly contentClient: FetchContentClient;

  constructor(config: GmailConfig, options: { publisher?: StorageBridgeEventPublisher; contentClient?: FetchContentClient } = {}) {
    this.config = validateConfig(config);
    this.contentClient = options.contentClient ?? {};
    if (options.publisher) this.bridge = new GmailBridge(this.config, options.publisher);
  }

  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    if (!this.bridge) throw new Error('GmailAdapter requires a publisher to handle notifications');
  ...
  mapNangoSyncRecord(record: NangoSyncRecord): StorageBridgeEvent {
    if (!this.nangoFallbackSyncName) throw new Error('Gmail does not declare a Nango scheduled-sync fallback');
    ...
    return {
      eventId: "gmail" + ':nango:' + id + ':' + occurredAt,
      occurredAt,
      ...
      metadata: { provider: "gmail", providerConfigKey: this.config.providerConfigKey, nango: record as never },
