...
import type { GmailConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: GmailConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "/gmail/v1/users/{account}/history", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: GmailConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "gmail" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "gmail",
    ...
    metadata: { provider: "gmail", providerConfigKey: config.providerConfigKey, delta: record },
