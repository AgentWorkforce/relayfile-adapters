...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${gmailIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'gmail' || digestEventPath(event) === '/gmail' || digestEventPath(event).startsWith('gmail/') || digestEventPath(event).startsWith('/gmail/'))
  );
}
...
function pastTense(event: DigestChangeEvent): string {
  ...
  if (hasActionVerb(action, 'create|created|add|added|write|written|receive|received')) {
    return 'was created';
  }
  if (hasActionVerb(action, 'send|sent')) {
    return 'was sent';
  }
