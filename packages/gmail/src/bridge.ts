...
import type { GmailConfig, JsonObject, JsonValue, ProviderNotification, StorageBridgeChangeType, StorageBridgeEvent, StorageBridgeEventPublisher } from './types.js';

const SOURCE: string = 'gmail';
...
export function validateWebhookRequest(notification: ProviderNotification, configInput: GmailConfig): boolean {
...
export function normalizeNotification(notification: ProviderNotification, configInput: GmailConfig): StorageBridgeEvent[] {
  const config = validateConfig(configInput);
...
export class GmailBridge {
  ...
  constructor(config: GmailConfig, publisher: StorageBridgeEventPublisher) {
  ...
  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    ...
    if (!validateWebhookRequest(notification, this.config)) {
      throw new Error('Gmail webhook signature validation failed');
    }
...
function expandProviderPayloads(notification: ProviderNotification, config: GmailConfig, detectedAt: string): JsonObject[] {
...
function toEvent(payload: JsonObject, config: GmailConfig, detectedAt: string, index: number): StorageBridgeEvent {
...
function normalizeGoogleDrive(notification: ProviderNotification, config: GmailConfig): JsonObject[] {
  const headers = normalizeHeaders(notification.headers ?? {});
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGraph(notification: ProviderNotification, config: GmailConfig, graphSource: 'sharepoint' | 'onedrive'): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeDropbox(notification: ProviderNotification, config: GmailConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGmail(notification: ProviderNotification, config: GmailConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
  const message = objectAt(body, 'message') ?? body;
...
function normalizeBox(notification: ProviderNotification, config: GmailConfig): JsonObject[] {
...
function normalizePostgres(notification: ProviderNotification, config: GmailConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
