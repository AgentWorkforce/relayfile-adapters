...
export const digest: DigestHandler = createDigestHandler({
  provider: "shopify",
  identify: (canonicalPath) => shopifyIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'shopify' || digestEventPath(event) === '/shopify' || digestEventPath(event).startsWith('shopify/') || digestEventPath(event).startsWith('/shopify/'))
  );
}
...
function shopifyIdentifier(path: string): string {
  ...
  if (path.includes('/orders/')) return `order ${label}`;
  if (path.includes('/products/')) return `product ${label}`;
  if (path.includes('/customers/')) return `customer ${label}`;
  if (path.includes('/fulfillments/')) return `fulfillment ${label}`;
  return label;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('fulfill|fulfilled');
const ACTION_VERB_PATTERN_3 = actionVerbRegex('cancel|canceled|cancelled');
const ACTION_VERB_PATTERN_4 = actionVerbRegex('delete|deleted|remove|removed');
const ACTION_VERB_PATTERN_5 = actionVerbRegex('paid');
