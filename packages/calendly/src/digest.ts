...
export const digest: DigestHandler = createDigestHandler({
  provider: "calendly",
  identify: (canonicalPath) => calendlyIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'calendly' || digestEventPath(event) === '/calendly' || digestEventPath(event).startsWith('calendly/') || digestEventPath(event).startsWith('/calendly/'))
  );
}
...
function calendlyIdentifier(path: string): string {
  const segments = path.split('/').filter(Boolean);
  const segment = segments.at(-1) ?? path;
  const basename = segment.replace(/\.[^.]+$/u, '');
  const decoded = decodePathLeafId(basename);

  if (path.includes('/scheduled-events/')) return `event ${decoded}`;
  if (path.includes('/invitees/')) return `invitee ${decoded}`;
  if (path.includes('/event-types/')) return `event type ${decoded}`;
  return decoded;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('cancel|canceled|cancelled');
...
function decodePathLeafId(value: string): string {
  try {
