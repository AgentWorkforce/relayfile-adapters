...
export async function fetchThread(input: ThreadFetchInput): Promise<ThreadExpansion> {
  const ticketResource = await readTicketResource(input);
  if (!ticketResource) {
...
async function readTicketResource(input: ThreadFetchInput): Promise<{ data: unknown } | null> {
  const path = input.event.resource.path;
  if (/^\/zendesk\/tickets\/[^/]+\.json$/u.test(path)) {
  ...
  const match = path.match(/^\/zendesk\/tickets\/([^/]+)\/comments\/[^/]+\.json$/u);
  return match?.[1]
    ? await input.client.readResource(input.workspace, `/zendesk/tickets/${match[1]}.json`)
    : null;
}
...
function normalizeThreadItem(value: unknown): ThreadItem {
  ...
  const authorId = stringifyId(authorRecord?.id) ?? 'unknown';
  const displayName = redactThreadText(readString(authorRecord?.name) ?? authorId) || authorId;
  return {
    id: stringifyId(record?.id) ?? 'thread-item',
