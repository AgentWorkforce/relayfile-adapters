...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${zendeskIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasCanonicalPath(event: DigestChangeEvent): event is DigestChangeEvent & { canonicalPath: string } {
  return (
    ...
    && (event.canonicalPath === 'zendesk' || event.canonicalPath === '/zendesk' || event.canonicalPath.startsWith('zendesk/') || event.canonicalPath.startsWith('/zendesk/'))
  );
}
