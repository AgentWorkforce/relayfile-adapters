...
import type { SharepointConfig, JsonObject, JsonValue, ProviderNotification, StorageBridgeChangeType, StorageBridgeEvent, StorageBridgeEventPublisher } from './types.js';

const SOURCE: string = 'sharepoint';
...
export function validateWebhookRequest(notification: ProviderNotification, configInput: SharepointConfig): boolean {
...
export function normalizeNotification(notification: ProviderNotification, configInput: SharepointConfig): StorageBridgeEvent[] {
  const config = validateConfig(configInput);
...
export class SharepointBridge {
  readonly config: SharepointConfig;
  readonly publisher: StorageBridgeEventPublisher;

  constructor(config: SharepointConfig, publisher: StorageBridgeEventPublisher) {
  ...
  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    ...
    if (!validateWebhookRequest(notification, this.config)) {
      throw new Error('Sharepoint webhook signature validation failed');
    }
...
function expandProviderPayloads(notification: ProviderNotification, config: SharepointConfig, detectedAt: string): JsonObject[] {
...
function toEvent(payload: JsonObject, config: SharepointConfig, detectedAt: string, index: number): StorageBridgeEvent {
...
function normalizeGoogleDrive(notification: ProviderNotification, config: SharepointConfig): JsonObject[] {
  const headers = normalizeHeaders(notification.headers ?? {});
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGraph(notification: ProviderNotification, config: SharepointConfig, graphSource: 'sharepoint' | 'onedrive'): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeDropbox(notification: ProviderNotification, config: SharepointConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGmail(notification: ProviderNotification, config: SharepointConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
  const message = objectAt(body, 'message') ?? body;
...
function normalizeBox(notification: ProviderNotification, config: SharepointConfig): JsonObject[] {
...
function normalizePostgres(notification: ProviderNotification, config: SharepointConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
