...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const canonical = !draftLike && parsedPath.id !== null && /^[A-Za-z0-9!._-]+$/.test(parsedPath.id);
  const resolvedOperation: WritebackOperation = operation ?? (content.trim().length === 0 ? 'delete' : canonical ? 'update' : 'create');
  const resource = parsedPath.resource === 'lifecycle' ? "subscriptions" : "items";
  const endpoint = resource === "subscriptions" ? "/v1.0/subscriptions" : "/v1.0/sites/{siteId}/drives/{driveId}/items/{itemId}";

  return {
    action: "sharepoint" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('SharePoint writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('SharePoint writeback requires a JSON object');
  }
  return parsed;
}
