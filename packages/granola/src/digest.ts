...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${granolaIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (
      digestEventPath(event) === 'granola'
      || digestEventPath(event) === '/granola'
      || digestEventPath(event).startsWith('granola/')
      || digestEventPath(event).startsWith('/granola/')
    )
  );
}
...
function hasDigestAliasDirectory(segments: readonly string[]): boolean {
  if ((segments[0] ?? '') !== 'granola') return false;

  for (let index = 1; index < segments.length - 1; index += 1) {
    const segment = segments[index];
    if (segment && DIGEST_ALIAS_SEGMENTS.has(segment)) {
      return true;
    }
  }
  return false;
...
function pastTense(event: DigestChangeEvent): string {
  ...
  if (hasActionVerb(action, ACTION_VERB_PATTERN_1)) return 'was created';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_2)) return 'was deleted';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_3)) return 'was merged';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_4)) return 'was archived';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_5)) return 'was closed';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_6)) return 'was completed';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_7)) return 'was canceled';
  if (hasActionVerb(action, ACTION_VERB_PATTERN_8)) return 'was resolved';
  return 'was updated';
}
