...
export const digest: DigestHandler = createDigestHandler({
  provider: "mixpanel",
  identify: (canonicalPath) => mixpanelIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (
      digestEventPath(event) === 'mixpanel'
      || digestEventPath(event) === '/mixpanel'
      || digestEventPath(event).startsWith('mixpanel/')
      || digestEventPath(event).startsWith('/mixpanel/')
    )
  );
}
...
function mixpanelIdentifier(path: string): string {
  ...
  if (path.includes('/events/')) return `event ${label}`;
  if (path.includes('/profiles/')) return `profile ${label}`;
  if (path.includes('/cohorts/')) return `cohort ${label}`;
  return label;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('merge|merged');
