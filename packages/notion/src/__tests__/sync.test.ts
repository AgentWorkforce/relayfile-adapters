...
describe('sync change detection', () => {
  it('builds a last_edited_time filter payload', () => {
    expect(buildSyncFilterPayload('2026-03-01T00:00:00.000Z')).toEqual({
      filter: {
  ...
  it('queries database changes with a last_edited_time filter', async () => {
    const paginate = vi.fn().mockResolvedValue([{ object: 'page', id: 'page-1', last_edited_time: '2026-03-02T00:00:00.000Z' }]);

    const result = await detectDatabaseChanges({ paginate } as never, 'db-1', '2026-03-01T00:00:00.000Z');

    expect(paginate).toHaveBeenCalledWith('POST', '/v1/databases/db-1/query', {
      body: {
        filter: {
          timestamp: 'last_edited_time',
          last_edited_time: { after: '2026-03-01T00:00:00.000Z' },
  ...
  it('paginates standalone search results before filtering by watermark', async () => {
    ...
    expect(paginate).toHaveBeenCalledWith('POST', '/v1/search', {
      body: {
    ...
    expect(result.pages.map((page) => page.id)).toEqual(['page-2']);
    expect(result.nextCursor).toBe('2026-03-06T00:00:00.000Z');
  });
});
