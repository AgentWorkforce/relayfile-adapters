...
import type { DropboxConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: DropboxConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "/2/files/list_folder/continue", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: DropboxConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "dropbox" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "dropbox",
    ...
    metadata: { provider: "dropbox", providerConfigKey: config.providerConfigKey, delta: record },
