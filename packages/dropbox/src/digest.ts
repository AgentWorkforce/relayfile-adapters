...
export const digest: DigestHandler = createDigestHandler({
  provider: "dropbox",
  identify: (canonicalPath) => dropboxIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'dropbox' || digestEventPath(event) === '/dropbox' || digestEventPath(event).startsWith('dropbox/') || digestEventPath(event).startsWith('/dropbox/'))
  );
}
...
const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|upload|uploaded|write|written');
