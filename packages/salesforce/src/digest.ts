...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${salesforceIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'salesforce' || digestEventPath(event) === '/salesforce' || digestEventPath(event).startsWith('salesforce/') || digestEventPath(event).startsWith('/salesforce/'))
  );
}
...
const ACTION_VERB_PATTERN_4 = actionVerbRegex('convert|converted');
