...
export const digest: DigestHandler = createDigestHandler({
  provider: "stripe",
  identify: (canonicalPath) => stripeIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'stripe' || digestEventPath(event) === '/stripe' || digestEventPath(event).startsWith('stripe/') || digestEventPath(event).startsWith('/stripe/'))
  );
}
...
function stripeIdentifier(path: string): string {
  const segments = path.split('/').filter(Boolean);
  const segment = segments.at(-1) ?? path;
  const basename = segment.replace(/\.[^.]+$/u, '');

  if (path.includes('/customers/')) return `customer ${basename}`;
  if (path.includes('/invoices/')) return `invoice ${basename}`;
  if (path.includes('/subscriptions/')) return `subscription ${basename}`;
  if (path.includes('/charges/')) return `charge ${basename}`;
  if (path.includes('/payment-intents/')) return `payment intent ${basename}`;
  return basename;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('succeed|succeeded');
const ACTION_VERB_PATTERN_3 = actionVerbRegex('fail|failed|payment_failed');
const ACTION_VERB_PATTERN_4 = actionVerbRegex('refund|refunded');
const ACTION_VERB_PATTERN_5 = actionVerbRegex('cancel|canceled|cancelled');
const ACTION_VERB_PATTERN_6 = actionVerbRegex('void|voided');
const ACTION_VERB_PATTERN_7 = actionVerbRegex('finalize|finalized');
const ACTION_VERB_PATTERN_8 = actionVerbRegex('paid');
