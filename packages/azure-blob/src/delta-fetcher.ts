...
import type { AzureBlobConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: AzureBlobConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "eventgrid:events", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: AzureBlobConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "azure-blob" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "azure-blob",
    ...
    metadata: { provider: "azure-blob", providerConfigKey: config.providerConfigKey, delta: record },
