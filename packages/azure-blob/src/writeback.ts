...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const resource = parsedPath.resource === 'lifecycle' ? "event-subscriptions" : "blobs";
  const endpoint = resource === "event-subscriptions" ? "/eventSubscriptions/{id}" : "/{container}/{name}";

  return {
    action: "azure-blob" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('Azure Blob Storage writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Azure Blob Storage writeback requires a JSON object');
  }
  return parsed;
}
