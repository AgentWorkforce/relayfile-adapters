import { AZUREBLOB_NANGO_FALLBACK_SYNC, validateConfig } from './config.js';
import { AzureBlobBridge } from './bridge.js';
...
import type { AzureBlobConfig, FetchContentClient, NangoSyncRecord, ProviderNotification, StorageBridgeEvent, StorageBridgeEventPublisher, WritebackOperation } from './types.js';

export class AzureBlobAdapter {
  readonly slug = "azure-blob";
  readonly source = "azure-blob";
  readonly resources = resources;
  readonly nangoFallbackSyncName = AZUREBLOB_NANGO_FALLBACK_SYNC;
  readonly config: AzureBlobConfig;
  private readonly bridge?: AzureBlobBridge;
  private readonly contentClient: FetchContentClient;

  constructor(config: AzureBlobConfig, options: { publisher?: StorageBridgeEventPublisher; contentClient?: FetchContentClient } = {}) {
    this.config = validateConfig(config);
    this.contentClient = options.contentClient ?? {};
    if (options.publisher) this.bridge = new AzureBlobBridge(this.config, options.publisher);
  }

  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    if (!this.bridge) throw new Error('Azure Blob StorageAdapter requires a publisher to handle notifications');
  ...
  mapNangoSyncRecord(record: NangoSyncRecord): StorageBridgeEvent {
    if (!this.nangoFallbackSyncName) throw new Error('Azure Blob Storage does not declare a Nango scheduled-sync fallback');
    ...
    return {
      eventId: "azure-blob" + ':nango:' + id + ':' + occurredAt,
      occurredAt,
      ...
      metadata: { provider: "azure-blob", providerConfigKey: this.config.providerConfigKey, nango: record as never },
