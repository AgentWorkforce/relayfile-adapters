...
import type { GcsConfig, JsonObject, JsonValue, ProviderNotification, StorageBridgeChangeType, StorageBridgeEvent, StorageBridgeEventPublisher } from './types.js';

const SOURCE: string = 'gcs';
...
export function validateWebhookRequest(notification: ProviderNotification, configInput: GcsConfig): boolean {
...
export function normalizeNotification(notification: ProviderNotification, configInput: GcsConfig): StorageBridgeEvent[] {
  const config = validateConfig(configInput);
...
export class GcsBridge {
  ...
  constructor(config: GcsConfig, publisher: StorageBridgeEventPublisher) {
  ...
  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    ...
    if (!validateWebhookRequest(notification, this.config)) {
      throw new Error('Gcs webhook signature validation failed');
    }
...
function expandProviderPayloads(notification: ProviderNotification, config: GcsConfig, detectedAt: string): JsonObject[] {
...
function toEvent(payload: JsonObject, config: GcsConfig, detectedAt: string, index: number): StorageBridgeEvent {
...
function normalizeGoogleDrive(notification: ProviderNotification, config: GcsConfig): JsonObject[] {
  const headers = normalizeHeaders(notification.headers ?? {});
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGraph(notification: ProviderNotification, config: GcsConfig, graphSource: 'sharepoint' | 'onedrive'): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeDropbox(notification: ProviderNotification, config: GcsConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGmail(notification: ProviderNotification, config: GcsConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
  const message = objectAt(body, 'message') ?? body;
...
function normalizeBox(notification: ProviderNotification, config: GcsConfig): JsonObject[] {
...
function normalizePostgres(notification: ProviderNotification, config: GcsConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
