...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const resource = parsedPath.resource === 'lifecycle' ? "notifications" : "objects";
  const endpoint = resource === "notifications" ? "/storage/v1/b/{bucket}/notificationConfigs" : "/storage/v1/b/{bucket}/o/{name}";

  return {
    action: "gcs" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('Google Cloud Storage writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Google Cloud Storage writeback requires a JSON object');
  }
  return parsed;
}
