...
export const digest: DigestHandler = createDigestHandler({
  provider: "postgres",
  identify: (canonicalPath) => postgresIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'postgres' || digestEventPath(event) === '/postgres' || digestEventPath(event).startsWith('postgres/') || digestEventPath(event).startsWith('/postgres/'))
  );
}
...
function postgresIdentifier(path: string): string {
  const segments = path.split('/').filter(Boolean);
  // postgres/<db>/<schema>/<table>/<pk>.json
  const table = segments.length >= 4 ? segments[3] : null;
  const pk = segments.at(-1)?.replace(/\.json$/u, '') ?? '';
  return table ? `row ${table}/${pk}` : `row ${pk}`;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('insert|inserted|create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('truncate|truncated');
