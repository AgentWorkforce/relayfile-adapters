...
import type { PostgresConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: PostgresConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "LISTEN", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: PostgresConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "postgres" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "postgres",
    ...
    metadata: { provider: "postgres", providerConfigKey: config.providerConfigKey, delta: record },
