...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const endpoint = resource === "subscriptions" ? "/v1.0/subscriptions" : "/v1.0/me/drive/items/{itemId}";

  return {
    action: "onedrive" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('OneDrive writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('OneDrive writeback requires a JSON object');
  }
  return parsed;
}
