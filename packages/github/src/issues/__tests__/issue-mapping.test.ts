...
describe('issue mapping', () => {
  ...
  it('mapIssue produces correct JSON shape', () => {
    ...
    assert.strictEqual(mapped.vfsPath, 'issues/10__track-adapter-issue-ingestion-coverage/meta.json');
  });
  ...
  it('ingestIssue writes meta.json and comments', async () => {
    ...
    const meta = JSON.parse(writes.get('/github/repos/octocat/hello-world/issues/10__track-adapter-issue-ingestion-coverage/meta.json') ?? '');
    ...
    const comment7001 = JSON.parse(writes.get('/github/repos/octocat/hello-world/issues/10__track-adapter-issue-ingestion-coverage/comments/7001/meta.json') ?? '');
  ...
  it('retains existing repo and issue index rows across repeated issue ingestion', async () => {
    ...
    const secondProvider = createFixtureProvider({
      issue: {
        ...cloneFixture(mockIssuePayload),
        number: 11,
        title: 'Add a second issue fixture',
        created_at: '2026-03-26T10:00:00Z',
        updated_at: '2026-03-29T07:45:00Z',
      } as JsonObject,
      issueComments: [],
    });

    await ingestIssue(firstProvider.provider, mockRepoContext.owner, mockRepoContext.repo, 10, vfs);
    await ingestIssue(secondProvider.provider, mockRepoContext.owner, mockRepoContext.repo, 11, vfs);
  ...
  it('VFS paths are correct', () => {
    ...
    assert.strictEqual(issueMapping.vfsPath, 'issues/10__track-adapter-issue-ingestion-coverage/meta.json');
    assert.strictEqual(commentMapping.vfsPath, 'issues/10/comments/7001/meta.json');
    assert.strictEqual(
