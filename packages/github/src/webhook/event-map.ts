...
export interface WebhookAdapter {
  ...
  ingestCommitStatus(payload: Record<string, unknown>): Promise<IngestResult>;
...
export const EVENT_MAP: Record<string, WebhookHandler> = {
  ...
  'pull_request.ready_for_review': (adapter, payload) => adapter.updatePullRequest(payload),
  'pull_request.labeled': (adapter, payload) => adapter.updatePullRequest(payload),
  'pull_request.unlabeled': (adapter, payload) => adapter.updatePullRequest(payload),
  ...
  status: (adapter, payload) => adapter.ingestCommitStatus(payload),
