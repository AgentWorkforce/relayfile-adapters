...
test('layoutManifest exposes Slack resources with canonical aliases and writeback schema pointers', () => {
  const manifest = layoutManifest();

  assert.equal(manifest.provider, 'slack');
  assert.deepEqual(manifest.aliasSegments, ['by-id', 'by-name']);

  for (const resource of manifest.resources) {
    assert.ok(resource.path.startsWith('slack/'));
    ...
    for (const writeback of resource.writebackResources) {
      assert.ok(writeback.path.startsWith('slack/'));
      assert.doesNotMatch(writeback.path, /^\//u);
      assert.match(writeback.schemaId, /^slack\/[a-z-]+$/u);
    }
  }
});
