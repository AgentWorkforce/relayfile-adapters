...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const canonical = !draftLike && parsedPath.id !== null && /^[0-9]+$/.test(parsedPath.id);
  const resolvedOperation: WritebackOperation = operation ?? (content.trim().length === 0 ? 'delete' : canonical ? 'update' : 'create');
  const resource = parsedPath.resource === 'lifecycle' ? "webhooks" : "files";
  const endpoint = resource === "webhooks" ? "/2.0/webhooks" : "/2.0/files/content";

  return {
    action: "box" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('Box writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Box writeback requires a JSON object');
  }
  return parsed;
}
