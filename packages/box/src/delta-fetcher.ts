...
import type { BoxConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: BoxConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "/2.0/events", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: BoxConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "box" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "box",
    ...
    metadata: { provider: "box", providerConfigKey: config.providerConfigKey, delta: record },
