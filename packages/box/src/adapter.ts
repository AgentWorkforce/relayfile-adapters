import { BOX_NANGO_FALLBACK_SYNC, validateConfig } from './config.js';
import { BoxBridge } from './bridge.js';
...
import type { BoxConfig, FetchContentClient, NangoSyncRecord, ProviderNotification, StorageBridgeEvent, StorageBridgeEventPublisher, WritebackOperation } from './types.js';

export class BoxAdapter {
  readonly slug = "box";
  readonly source = "box";
  readonly resources = resources;
  readonly nangoFallbackSyncName = BOX_NANGO_FALLBACK_SYNC;
  readonly config: BoxConfig;
  private readonly bridge?: BoxBridge;
  private readonly contentClient: FetchContentClient;

  constructor(config: BoxConfig, options: { publisher?: StorageBridgeEventPublisher; contentClient?: FetchContentClient } = {}) {
    this.config = validateConfig(config);
    this.contentClient = options.contentClient ?? {};
    if (options.publisher) this.bridge = new BoxBridge(this.config, options.publisher);
  }

  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    if (!this.bridge) throw new Error('BoxAdapter requires a publisher to handle notifications');
  ...
  mapNangoSyncRecord(record: NangoSyncRecord): StorageBridgeEvent {
    if (!this.nangoFallbackSyncName) throw new Error('Box does not declare a Nango scheduled-sync fallback');
    ...
    return {
      eventId: "box" + ':nango:' + id + ':' + occurredAt,
      occurredAt,
      ...
      metadata: { provider: "box", providerConfigKey: this.config.providerConfigKey, nango: record as never },
