...
import type { BoxConfig, JsonObject, JsonValue, ProviderNotification, StorageBridgeChangeType, StorageBridgeEvent, StorageBridgeEventPublisher } from './types.js';

const SOURCE: string = 'box';
...
export function validateWebhookRequest(notification: ProviderNotification, configInput: BoxConfig): boolean {
...
export function normalizeNotification(notification: ProviderNotification, configInput: BoxConfig): StorageBridgeEvent[] {
  const config = validateConfig(configInput);
...
export class BoxBridge {
  readonly config: BoxConfig;
  readonly publisher: StorageBridgeEventPublisher;

  constructor(config: BoxConfig, publisher: StorageBridgeEventPublisher) {
  ...
  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    ...
    if (!validateWebhookRequest(notification, this.config)) {
      throw new Error('Box webhook signature validation failed');
    }
...
function expandProviderPayloads(notification: ProviderNotification, config: BoxConfig, detectedAt: string): JsonObject[] {
...
function toEvent(payload: JsonObject, config: BoxConfig, detectedAt: string, index: number): StorageBridgeEvent {
...
function normalizeGoogleDrive(notification: ProviderNotification, config: BoxConfig): JsonObject[] {
  const headers = normalizeHeaders(notification.headers ?? {});
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGraph(notification: ProviderNotification, config: BoxConfig, graphSource: 'sharepoint' | 'onedrive'): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeDropbox(notification: ProviderNotification, config: BoxConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGmail(notification: ProviderNotification, config: BoxConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
  const message = objectAt(body, 'message') ?? body;
...
function normalizeBox(notification: ProviderNotification, config: BoxConfig): JsonObject[] {
...
function normalizePostgres(notification: ProviderNotification, config: BoxConfig): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
