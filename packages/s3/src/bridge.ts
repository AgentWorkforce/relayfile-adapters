...
import type { S3Config, JsonObject, JsonValue, ProviderNotification, StorageBridgeChangeType, StorageBridgeEvent, StorageBridgeEventPublisher } from './types.js';

const SOURCE: string = 's3';
...
export function validateWebhookRequest(notification: ProviderNotification, configInput: S3Config): boolean {
...
export function normalizeNotification(notification: ProviderNotification, configInput: S3Config): StorageBridgeEvent[] {
  const config = validateConfig(configInput);
...
export class S3Bridge {
  ...
  constructor(config: S3Config, publisher: StorageBridgeEventPublisher) {
  ...
  async handleNotification(notification: ProviderNotification): Promise<StorageBridgeEvent[]> {
    ...
    if (!validateWebhookRequest(notification, this.config)) {
      throw new Error('S3 webhook signature validation failed');
    }
...
function expandProviderPayloads(notification: ProviderNotification, config: S3Config, detectedAt: string): JsonObject[] {
...
function toEvent(payload: JsonObject, config: S3Config, detectedAt: string, index: number): StorageBridgeEvent {
...
function normalizeGoogleDrive(notification: ProviderNotification, config: S3Config): JsonObject[] {
  const headers = normalizeHeaders(notification.headers ?? {});
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGraph(notification: ProviderNotification, config: S3Config, graphSource: 'sharepoint' | 'onedrive'): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeDropbox(notification: ProviderNotification, config: S3Config): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
...
function normalizeGmail(notification: ProviderNotification, config: S3Config): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
  const message = objectAt(body, 'message') ?? body;
...
function normalizeBox(notification: ProviderNotification, config: S3Config): JsonObject[] {
...
function normalizePostgres(notification: ProviderNotification, config: S3Config): JsonObject[] {
  const body = isObject(notification.body) ? notification.body : {};
