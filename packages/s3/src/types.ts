...
export type StorageBridgeEvent = Omit<CoreStorageBridgeEvent, 'source' | 'metadata'> & { readonly source: 's3'; readonly metadata: JsonObject };
...
export interface FetchContentClient {
  getObject?(event: StorageBridgeEvent, config: S3Config): Promise<string | Uint8Array | null>;
}
