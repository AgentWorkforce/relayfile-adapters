...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const resource = parsedPath.resource === 'lifecycle' ? "queues" : "objects";
  const endpoint = resource === "queues" ? "sqs:ReceiveMessage" : "s3:PutObject";

  return {
    action: "s3" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('Amazon S3 writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Amazon S3 writeback requires a JSON object');
  }
  return parsed;
}
