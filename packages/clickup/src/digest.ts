...
export const digest: DigestHandler = createDigestHandler({
  provider: "clickup",
  identify: (canonicalPath) => clickupIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'clickup' || digestEventPath(event) === '/clickup' || digestEventPath(event).startsWith('clickup/') || digestEventPath(event).startsWith('/clickup/'))
  );
}
...
function clickupIdentifier(path: string): string {
  ...
  if (path.includes('/folders/')) return `folder ${id}`;
  if (path.includes('/spaces/')) return `space ${id}`;
  return id;
}
...
const ACTION_VERB_PATTERN_3 = actionVerbRegex('close|closed|complete|completed|resolve|resolved|done');
