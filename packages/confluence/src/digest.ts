...
export const digest: DigestHandler = createDigestHandler({
  provider: "confluence",
  identify: (canonicalPath) => confluenceIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (
      digestEventPath(event) === 'confluence'
      || digestEventPath(event) === '/confluence'
      || digestEventPath(event).startsWith('confluence/')
      || digestEventPath(event).startsWith('/confluence/')
    )
  );
}
...
function confluenceIdentifier(path: string): string {
  ...
  if (path.includes('/pages/')) return `page ${id}`;
...
const ACTION_VERB_PATTERN_2 = actionVerbRegex('restore|restored|unarchive|unarchived');
const ACTION_VERB_PATTERN_3 = actionVerbRegex('archive|archived');
const ACTION_VERB_PATTERN_4 = actionVerbRegex('trash|trashed');
