...
export const digest: DigestHandler = createDigestHandler({
  ...
  identify: (canonicalPath) => googleCalendarIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'google-calendar'
      || digestEventPath(event) === '/google-calendar'
      || digestEventPath(event).startsWith('google-calendar/')
      || digestEventPath(event).startsWith('/google-calendar/'))
  );
}
...
function googleCalendarIdentifier(path: string): string {
  ...
  if (path.includes('/calendars/')) return `calendar ${id}`;
  return id;
}

const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('cancel|cancelled|canceled');
