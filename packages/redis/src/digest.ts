...
export const digest: DigestHandler = async (ctx) => {
  ...
  const bullets = events
    ...
    .map((event) => {
      ...
      return {
        text: `${redisIdentifier(canonicalPath)} ${pastTense(event)}`,
        canonicalPath,
      };
    });
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'redis' || digestEventPath(event) === '/redis' || digestEventPath(event).startsWith('redis/') || digestEventPath(event).startsWith('/redis/'))
  );
}
