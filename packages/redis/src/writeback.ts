...
export function resolveWritebackRequest(path: string, content: string, operation?: WritebackOperation): ProviderWritebackRequest {
  ...
  const resource = parsedPath.resource === 'lifecycle' ? "listeners" : "keys";
  const endpoint = resource === "listeners" ? "PSUBSCRIBE __keyspace@{db}__:*" : "SET/HSET/ZADD/DEL";

  if (resource === "keys") {
    return buildRedisRequest(payload, resolvedOperation, parsedPath.id);
  }
  return {
    action: "redis" + '.' + resource + '.' + resolvedOperation,
...
function parseJsonObject(content: string): JsonObject {
  ...
  try {
    ...
    throw new Error('Redis writeback requires valid JSON: ' + (error instanceof Error ? error.message : String(error)));
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Redis writeback requires a JSON object');
  }
  return parsed;
}
...
function buildRedisRequest(payload: JsonObject, operation: WritebackOperation, id: string | null): ProviderWritebackRequest {
  const type = typeof payload.type === 'string' ? payload.type : 'string';
  const method = operation === 'delete' ? 'DELETE' : operation === 'create' ? 'POST' : 'PATCH';
  const endpoint = operation === 'delete' ? 'DEL' : type === 'hash' ? 'HSET' : type === 'zset' ? 'ZADD' : 'SET';
  return {
    action: 'redis.keys.' + operation,
    operation,
    method,
    endpoint,
    resource: 'keys',
    resourceId: id ?? (typeof payload.key === 'string' ? payload.key : null),
    body: operation === 'delete' ? null : payload,
  };
}
