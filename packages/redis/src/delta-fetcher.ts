...
import type { RedisConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: RedisConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "PSUBSCRIBE", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: RedisConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "redis" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "redis",
    ...
    metadata: { provider: "redis", providerConfigKey: config.providerConfigKey, delta: record },
