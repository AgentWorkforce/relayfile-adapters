...
export const digest: DigestHandler = createDigestHandler({
  provider: "sendgrid",
  identify: (canonicalPath) => sendgridIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'sendgrid' || digestEventPath(event) === '/sendgrid' || digestEventPath(event).startsWith('sendgrid/') || digestEventPath(event).startsWith('/sendgrid/'))
  );
}
...
const ACTION_VERB_PATTERN_1 = actionVerbRegex('create|created|add|added|write|written|process|processed');
const ACTION_VERB_PATTERN_2 = actionVerbRegex('deliver|delivered');
const ACTION_VERB_PATTERN_3 = actionVerbRegex('bounce|bounced|drop|dropped');
