...
export const digest: DigestHandler = createDigestHandler({
  provider: "pipedrive",
  identify: (canonicalPath) => pipedriveIdentifier(canonicalPath),
  alias: { segments: [] },
...
function hasDigestPath(event: DigestChangeEvent): boolean {
  return (
    ...
    && (digestEventPath(event) === 'pipedrive' || digestEventPath(event) === '/pipedrive' || digestEventPath(event).startsWith('pipedrive/') || digestEventPath(event).startsWith('/pipedrive/'))
  );
}
...
const ACTION_VERB_PATTERN_3 = actionVerbRegex('won');
const ACTION_VERB_PATTERN_4 = actionVerbRegex('lost');
