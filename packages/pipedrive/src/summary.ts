...
export function buildSummary(payload: Record<string, unknown>): EventSummary {
  const deal = readRecord(payload.current) ?? readRecord(payload.data) ?? payload;
  const title = truncateText(readString(deal.title), MAX_TITLE_LENGTH);
  const status = truncateText(readString(deal.status), MAX_TEXT_FIELD_LENGTH);
  const pipeline = referenceValue(deal.pipeline_id);
  const tags = limitTexts(
...
function trimArray(summary: EventSummary, key: 'tags'): boolean {
...
function referenceValue(value: unknown): string | undefined {
  ...
  return readString(record?.name) ?? readString(record?.value) ?? stringifyNumber(record?.id);
}
