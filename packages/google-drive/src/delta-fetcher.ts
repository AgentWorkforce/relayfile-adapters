...
import type { GoogleDriveConfig, JsonObject, StorageBridgeEvent } from './types.js';
...
export function buildDeltaFetchRequest(config: GoogleDriveConfig, cursor: string | null = null): DeltaFetchRequest {
  const headers: Record<string, string> = { accept: 'application/json' };
  if (config.accessToken) headers.authorization = 'Bearer ' + config.accessToken;
  return { endpoint: cursor ?? "/drive/v3/changes", cursor, headers };
}

export function mapDeltaRecord(record: JsonObject, config: GoogleDriveConfig, detectedAt = new Date().toISOString()): StorageBridgeEvent {
  ...
  return {
    eventId: "google-drive" + ':delta:' + id + ':' + occurredAt,
    occurredAt,
    detectedAt,
    source: "google-drive",
    ...
    metadata: { provider: "google-drive", providerConfigKey: config.providerConfigKey, delta: record },
