...
function hasCanonicalPath(event: DigestChangeEvent): event is DigestChangeEvent & { canonicalPath: string } {
  return (
    ...
    && (event.canonicalPath === 'google-drive' || event.canonicalPath === '/google-drive' || event.canonicalPath.startsWith('google-drive/') || event.canonicalPath.startsWith('/google-drive/'))
  );
}
