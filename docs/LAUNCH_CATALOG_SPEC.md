...
2. **Provider matrix** — every adapter inherits OAuth from `@relayfile/provider-{nango,pipedream,composio,clerk}`. We never write OAuth N times. Cross-reference: [NangoHQ/integration-templates](https://github.com/NangoHQ/integration-templates) — ~110 first-party templates we lift mapping hints from.
...
- **Paperclip** — citation tool with no public API of note. We ship Semantic Scholar + arXiv instead.
- **OCI** — covered by S3-compatible client; can be a config flag on the s3 adapter rather than a separate row.

If marketing wants 60+ for the splash, stretch rows: `oci`, `webflow`, `airtable`, `mailchimp`, `shopify`, `quickbooks`, `mysql`, `redshift`, `bigquery` — all have Nango templates and OpenAPI specs available.

## 5. Tier-1 adapter spec sheets

Compact spec per T1 adapter — enough to file the YAML mapping without further research. All paths are VFS paths under the workspace root; OAuth is handled by the Nango/Pipedream/Composio provider unless noted.
...
- Reference implementation for everything. Don't change.
...
### 8.1 Tooling: `relayfile-adapter` CLI (lands Mon)

The repo already uses npm workspaces (`"packageManager": "npm@10.8.2"`) with turbo for build orchestration, and `@relayfile/adapter-core` already exposes a `cli.ts` entry point with `ingest`, `generate`, and `drift` subcommands. We extend it with a `scaffold` subcommand exposed as a workspace bin so the launch wave can run from any package directory.
...
{
  "bin": {
    "relayfile-adapter": "./dist/cli.js"
  },
  "scripts": {
    "scaffold": "node ./dist/cli.js scaffold"
  }
}
```

In the workspace root `package.json`, add a forwarding script:

```json
{
  "scripts": {
    "gen:adapter": "npm exec --workspace=@relayfile/adapter-core -- relayfile-adapter scaffold",
    "catalog:audit": "npm exec --workspace=@relayfile/adapter-core -- relayfile-adapter audit"
  }
}
```
...
```bash
npm run gen:adapter -- jira \
  --openapi https://dac-static.atlassian.com/cloud/jira/platform/swagger-v3.v3.json \
  --auth oauth2 \
  --webhook-sig hmac-sha256:X-Atlassian-Webhook-Identifier \
  --nango-template jira
```

This emits `packages/jira/` with: `package.json` + `tsconfig.json` (copied from `templates/adapter/`), `src/{index,adapter,types,path-mapper,writeback}.ts`, `src/webhook/verify.ts` with passing+tampered fixtures, `jira.mapping.yaml` pre-filled with `adapter.{name,version,source.openapi}`, generated `types.ts` from the OpenAPI schemas (via existing `types-generator.ts`), a `templates/jira.hints.yaml` lifted from the Nango template directory, plus a row appended to `docs/CATALOG.md` and `docs/MIRAGE_PARITY.md`.

### 8.2 Day-by-day

| Day | Deliverable |
|---|---|
| **Mon** | Land `scaffold` subcommand + `templates/adapter/` skeleton. `npm run gen:adapter -- foo` produces a passing-build package with TODO markers for path mapping. |
...
| **Sun** | Launch hygiene: every adapter gets a one-paragraph README, a `mirage-vs-relayfile.md` row, and a smoke test in CI. Cut `@relayfile/adapter-*` versions and publish to npm. |
...
### 8.3 Versioning + publish

All adapters are independently versioned packages under `@relayfile/adapter-*`. Use `npm publish --workspaces --access public` from the root for the launch cut, gated on `npm run build` + `npm run test` passing across the whole workspace via turbo.
...
CI gate: `npm run catalog:audit` asserts that the published catalog count ≥ Mirage's tracked count (manually maintained in `docs/MIRAGE_PARITY.md` and grepped from their docs weekly).
