...
| Linear | `@relayfile/adapter-linear/writeback` | Linear GraphQL `POST /graphql` mutations for issue/comment/agent-activity writes; Linear project writeback actions `create-project`, `update-project`, `add-issues-to-project`, and `archive-project` | Call Nango proxy with `linear-relay`; inspect GraphQL `errors` plus mutation `success` for GraphQL writes, and route project action requests to the companion `linear-relay` action endpoints. |
